...
- **p_reach shadowban target** (default `random`): ΜΟΝΟ για `shadowban`. ΠΟΙΟΙ agents τιμωρούνται. `random` = τυχαίο υποσύνολο μεγέθους *shadowban fraction*. `extreme` = οι πιο ακραίοι agents (μεγαλύτερο `|αρχική γνώμη|`, deterministic, ties by index) — «targeted shadow-ban» που σιωπά τις πιο έντονες φωνές. `balanced_extreme` = εναλλάξ ο πιο αρνητικός και ο πιο θετικός (π.χ. ένας +2 κι ένας −2), sign-symmetric — ξεχωρίζει «ένταση» από «κατεύθυνση» (χρήσιμο όταν το extreme run θρόττλαρε μονόπλευρους ακραίους). Με `enforcement = suppress` δίνει καθαρή **targeted-vs-neutral** single-seed σύγκριση (P21: μετράει αν η σίγαση των *επιδραστικών* agents αλλάζει τη δυναμική, σε αντίθεση με τυχαίους/ουδέτερους).
